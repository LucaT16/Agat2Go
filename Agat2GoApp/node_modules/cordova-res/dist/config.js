...
async function run(configPath, resourcesDirectory, sources, resources) {
    const colors = sources.filter((source) => source.type === "color" /* COLOR */);
    const config = await read(configPath);
    const androidPlatformElement = resolvePlatformElement(config.getroot(), "android" /* ANDROID */);
    if (colors.length > 0) {
...
function runConfig(configPath, resources, doc) {
...
function runResource(configPath, resource, container) {
